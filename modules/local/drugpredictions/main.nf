process DRUGPREDICTIONS {
    ...
    output:
    tuple val(meta+"."+algorithm), path("${meta.id}.${algorithm}.drug_predictions.tsv") , emit: drug_predictions
    tuple val(meta+"."+algorithm), path("${meta.id}.${algorithm}.csv"), emit: drugstone_download
